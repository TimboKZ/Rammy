...
class Rammy {
    ...
    initProject(data) {
        ...
        return Promise.resolve()
            ...
            .then(() => {
                if (fs.pathExistsSync(configPath)) throw new RammyError(`'${Constants.ConfigFileName}' already exists.`);
            })
                ...
                return gitPromise
                    ...
                    .then(texIgnoreContents => {
                        ...
                        if (ignoreExists) {
                            ...
                            return fs.appendFile(gitignorePath, texIgnoreContents)
                        }
                        else {
                            return fs.writeFile(gitignorePath, texIgnoreContents);
                        }
                    });
            })
    ...
    initModule(data) {
        ...
        return Promise.resolve()
            .then(() => Util.validate(schema, data))
            .then(() => path.join(data.directory, Constants.ConfigFileName))
    ...
    addModule(data) {
        ...
        return Promise.resolve()
            ...
            .then(() => {
                const moduleConfigPath = path.join(pathToModule, Constants.ConfigFileName);
     ...
     * @param {string} data.template Template name or path
     * @returns {Promise<void>}
     */
    createFile(data) {
        const schema = {
