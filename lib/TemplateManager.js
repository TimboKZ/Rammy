...
class TemplateManager {
    ...
    renderTemplate(data) {
        let output = '';
        const addLine = line => output += `${line}${os.EOL}`;
        return new Promise((resolve, reject) => {
            try {
                addLine(`%-- Generated using Rammy v${Util.getPackageVersion()} | github.com/TimboKZ/Rammy`);
                addLine(`%-- Module: ${data.templateData.module.name}`);
                addLine(`%-- Template: ${data.templateData.name}`);
