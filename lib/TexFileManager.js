...
class TexFileManager {
    ...
    createFileFromTemplate(data) {
        /** @var {{modules: Object<string, ModuleData>, allTemplates: Object<string, TemplateData>, allInputs: Object<string, InputData}} */
        let allModuleData;
        /** @var {TemplateData} */
        let templateData;
    ...
    renderTemplateLine(data) {
        ...
        const newLine = line.replace(this._inputLineRegex, (fullMatch, inputPath, type, value, trailing) => {
            ...
            if (['input', 'template'].includes(type)) {

                const component = data.templateData.module[`${type}s`][value];
                if (component) {
                    pathToRender = path.relative(targetFileDir, component.absPath);
                } else {
                    this.logger.warn(`Couldn't resolve ${type} name '${value}' on line ${data.lineNumber} - ignoring this line.`);
                    doReplace = false;
                    return;
                }
                ...
                switch (value) {
                    ...
                    case 'absolute':
                        ...
                        break;
                    default:
                        this.logger.warn(`Unknown path type '${value}' on line ${data.lineNumber} - ignoring this line.`);
                        doReplace = false;
                        return;
                }

            } else {
                this.logger.warn(`Unknown type '${type}' on line ${data.lineNumber} - ignoring this line.`);
                doReplace = false;
                return;
            }
