        ...
        return Promise.resolve()
            ...
            .then(moduleConfig => {
                ...
                if (!moduleConfig.module)
                    this.logger.warn(`Module '${moduleData.name}' doesn't have the 'module' property specified in its\
                     config. Are you sure it's a Rammy module?`);
                if (moduleData.name !== moduleConfig.name)
                    this.logger.warn(`There is a name mismatch for module '${moduleData.name}' - your project config\
                     says '${moduleData.name}' while module config says '${moduleConfig.name}'`);
